...
from rrgcn import RecurrentRGCN, RecurrentRGCN_Ori, RecurrentRGCN_Decoder, RecurrentRGCN_2025
from hyperparameter_range import *
...
def test(model, history_list, test_list, num_rels, num_nodes, use_cuda, all_ans_list, all_ans_r_list, model_name, static_graph, time_list, history_time_nogt, mode, predict_steps=1):
    ...
    # 修改循环次数为predict_steps
    total_steps = min(predict_steps, len(test_list)) if predict_steps > 0 else len(test_list)
    
    for time_idx in tqdm(range(total_steps)):  # 修改循环条件
        if time_idx < len(test_list):
            test_snap = test_list[time_idx]
        else:
            # 如果预测步数超过测试集长度，使用最后一个测试样本
            test_snap = test_list[-1]


    if args.multi_step:
        # all_tail_seq = sp.load_npz(
        #     '../data/{}/history/tail_history_{}.npz'.format(args.dataset, history_time_nogt))
        all_tail_seq = sp.load_npz(os.path.join(os.getcwd(), 'data', args.dataset, 'history', 'tail_history_{}.npz'.format(history_time_nogt)))
        # rel
        # all_rel_seq = sp.load_npz(
        #     '../data/{}/history/rel_history_{}.npz'.format(args.dataset, history_time_nogt))
        all_rel_seq = sp.load_npz(os.path.join(os.getcwd(), 'data', args.dataset, 'history', 'rel_history_{}.npz'.format(history_time_nogt)))
    ...
    for time_idx, test_snap in enumerate(tqdm(test_list)):
        ...
        if args.multi_step:
            ...
            all_tail_seq = sp.load_npz(os.path.join(os.getcwd(), 'data', args.dataset, 'history', 'tail_history_{}.npz'.format(time_list[time_idx])))
            seq_idx = histroy_data[:, 0] * num_rels * 2 + histroy_data[:, 1]
            tail_seq = torch.Tensor(all_tail_seq[seq_idx].todense())
            one_hot_tail_seq = tail_seq.masked_fill(tail_seq != 0, 1)
            # rel
            ...
            all_rel_seq = sp.load_npz(os.path.join(os.getcwd(), 'data', args.dataset, 'history', 'rel_history_{}.npz'.format(time_list[time_idx])))
            rel_seq_idx = histroy_data[:, 0] * num_nodes + histroy_data[:, 2]
            rel_seq = torch.Tensor(all_rel_seq[rel_seq_idx].todense())
            one_hot_rel_seq = rel_seq.masked_fill(rel_seq != 0, 1)
        if use_cuda:
        ...
        if args.multi_step:
            if not args.relation_evaluation:    
                ...
                predicted_snap = utils.construct_snap(test_triples.cpu() if torch.is_tensor(test_triples) else test_triples, num_nodes, num_rels, final_score.cpu() if torch.is_tensor(final_score) else final_score, args.topk) 
            else:
                #predicted_snap = utils.construct_snap_r(test_triples, num_nodes, num_rels, final_r_score, args.topk)
                predicted_snap = utils.construct_snap_r(test_triples.cpu() if torch.is_tensor(test_triples) else test_triples, num_nodes, num_rels, final_r_score.cpu() if torch.is_tensor(final_r_score) else final_r_score, args.topk)
            if len(predicted_snap):
                input_list.pop(0)
                input_list.append(predicted_snap)
        
        else:
            if time_idx < len(test_list) - 1:  # 防止索引越界
                input_list.pop(0)
                input_list.append(test_list[time_idx + 1])
        # 打印当前预测步数
        print(f"Step t+{time_idx+1} completed")
...
def run_experiment(args, history_len=None, n_layers=None, dropout=None, n_bases=None, angle=None, history_rate=None):
    ...
    time_interval = train_times[1]-train_times[0]  # 时间的间隔
    print("num_times", num_times, "--------------", time_interval)
    history_val_time_nogt = valid_times[0]  # 验证集从哪个时间点开始的
    history_test_time_nogt = test_times[0]  # 测试集从哪个时间点开始的
    if args.multi_step:  # False
    ...
    model_state_file = os.path.join(os.getcwd(), 'torch_saved', model_name)
    ...
    if args.add_static_graph:
        ...
        static_triples = np.array(_read_triplets_as_list(os.path.join(os.getcwd(), 'data', args.dataset, "e-w-graph.txt"), {}, {}, load_time=False))
    ...
    # create stat
    if args.name == 'RecurrentRGCN_Ori':
        model = RecurrentRGCN_Ori(args.decoder,
                            args.encoder,
                            num_nodes,
                            num_rels,
                            num_static_rels,
                            ...
                            args=args)
    elif args.name == 'RecurrentRGCN_Decoder':
        model = RecurrentRGCN_Decoder(args.decoder,
                            args.encoder,
                            num_nodes,
                            num_rels,
                            num_static_rels,
                            ...
                            analysis=args.run_analysis,
                            args=args)
    elif args.name == 'RecurrentRGCN_2025':
        model = RecurrentRGCN_2025(args.decoder,
                            args.encoder,
                            num_nodes,
                            num_rels,
                            num_static_rels,
                            ...
                            analysis=args.run_analysis,
                            args=args)
    elif args.name == 'RecurrentRGCN_FrozenREN':
        model = RecurrentRGCN_FrozenREN(args.decoder,
                        args.encoder,
                        num_nodes,
                        num_rels,
                        num_static_rels,
        ...
        # 加载预训练的REN权重
        if use_cuda:
            checkpoint = torch.load('path_to_pretrained_ren_model.pth', map_location=torch.device(args.gpu))
        else:
            checkpoint = torch.load('path_to_pretrained_ren_model.pth', map_location=torch.device('cpu'))
    
    # 加载base_model的权重
        model.base_model.load_state_dict(checkpoint['state_dict'])
    # 冻结REN参数
        model.freeze_ren_parameters()
    else:
    ...
    if use_cuda:
        ...
        torch.backends.cudnn.deterministic = True  # 类似随机种子
        model.cuda()
    ...
    if args.test and os.path.exists(model_state_file):
        ...
        for epoch in range(args.n_epochs):
            ...
            for train_sample_num in tqdm(idx):
                ...
                output_graph = build_sub_graph(num_nodes, num_rels, output[-1], use_cuda, args.gpu)
                ...
                all_tail_seq = sp.load_npz(os.path.join(os.getcwd(), 'data', args.dataset, 'history', 'tail_history_{}.npz'.format(train_times[train_sample_num])))
                seq_idx = histroy_data[:, 0] * num_rels * 2 + histroy_data[:, 1]
                tail_seq = torch.Tensor(all_tail_seq[seq_idx].todense())
                one_hot_tail_seq = tail_seq.masked_fill(tail_seq != 0, 1)
                # rel
                ...
                all_rel_seq = sp.load_npz(os.path.join(os.getcwd(), 'data', args.dataset, 'history', 'rel_history_{}.npz'.format(train_times[train_sample_num])))
                rel_seq_idx = histroy_data[:, 0] * num_nodes + histroy_data[:, 2]
                rel_seq = torch.Tensor(all_rel_seq[rel_seq_idx].todense())
                one_hot_rel_seq = rel_seq.masked_fill(rel_seq != 0, 1)
                if use_cuda:
                ...
                # loss_e, loss_r, loss_static, loss_diffusion = model.module.get_loss(history_glist, output[0], static_graph, one_hot_tail_seq, one_hot_rel_seq, use_cuda, output_graph)
                loss_e, loss_r, loss_static, loss_diffusion, loss_diffusion2 = model.get_loss(history_glist, output[0], static_graph, one_hot_tail_seq, one_hot_rel_seq, use_cuda, output_graph)
                #loss = args.task_weight*loss_e + (1-args.task_weight)*loss_r + loss_static + args.loss_weight * loss_diffusion
                loss = args.task_weight*loss_e + (1-args.task_weight)*loss_r + loss_static + args.lambda1 * loss_diffusion + args.lambda2 * loss_diffusion2
            ...
            # validation
            if epoch % args.evaluate_every == 0:# if epoch and epoch % args.evaluate_every == 0:
                mrr_raw, mrr_filter, mrr_raw_r, mrr_filter_r, hit_result_raw, hit_result_filter, hit_result_raw_r, hit_result_filter_r = test(model,
                                                                    train_list, 
                                                                    valid_list, 
                                                                    num_rels, 
...
if __name__ == '__main__':
    ...
    parser.add_argument("--predict-steps", type=int, default=0,
                        help="number of future steps to predict (0 means use all test steps)")
    ...
    #parser.add_argument("--loss-weight", type=float, default=1.0, help="loss weight of the diffusion model")
    parser.add_argument("--lambda1", type=float, default=0.1, help="lambda1 of the ent diffusion model")
    parser.add_argument("--lambda2", type=float, default=0.05, help="loss weight of the rel diffusion model")
    
    # configuration for optimal parameters
    parser.add_argument('--seed', dest="seed", default=123, type=int, help='Seed to reproduce results')


    parser.add_argument("--beta-end", type=float, default=0.05, help="learning rate")



    args = parser.parse_args()
    print(args)
    np.random.seed(args.seed)  # 这三句都写
